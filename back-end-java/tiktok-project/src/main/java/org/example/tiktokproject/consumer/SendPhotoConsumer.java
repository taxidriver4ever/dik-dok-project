...
public class SendPhotoConsumer {
    ...
    private void saveVideoData(PhotoAndVideo photoAndVideo, DetectionResponse detectionResponse) throws Exception {
        ...
        // 3. 设置作者信息
        mySQLVideo.setAuthor(photoAndVideo.getAuthor());
        video.setAuthor(mySQLVideo.getAuthor());

        // 4. 保存到数据库（允许重复URL）
        videoMapper.insertVideo(mySQLVideo);
        videoRepository.save(video);
        videoRedis.setInformation(photoAndVideo.getVideo());
    ...
    private Video createVideo(PhotoAndVideo photoAndVideo, DetectionResponse detectionResponse) {
        ...
        video.setUploadDateMilli(System.currentTimeMillis());
    ...
    private MySQLVideo convertToMySQLVideo(Video video) {
        ...
        mySQLVideo.setLikeCount(0);
        mySQLVideo.setViewCount(0);
        mySQLVideo.setShareCount(0);
        mySQLVideo.setCommentCount(0);
        return mySQLVideo;
    }
